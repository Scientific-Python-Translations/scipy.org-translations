...
SciPy 는 파이썬을 위한 오픈 소스(BSD 라이선스) 과학·수치 계산용 SW 의
모음입니다. 현재 특수 함수, 적분, 상미분 방정식(ODE) 솔버, 경사 최적화,
병렬 프로그래밍용 SW 도구, 실행 가속을 위한 표현식 단위 C++ 컴파일러 등을
지원합니다. 대략적인 기준을 말씀드리자면, 수치 연산에 관한 일반 교재(예: 잘 알려진
Numerical Recipes 시리즈)에서 다루어지는 내용은 SciPy 에 구현되어 있을
가능성이 높습니다.
...
## 어떻게 SciPy 가 빠를 수가 있나요? 파이썬 같은 인터프리터 언어로 작성되었다면.

사실, 연산 소요 시간이 중요한 반복문은 보통 C, C++, 또는 Fortran 으로
구현되어 있습니다. SciPy 의 일부는 <https://www.netlib.org/> 에서 자유롭게 사용될 수 있도록
무료로 제공되는 과학 계산 루틴 위에 얇은 코드 계층을 얹은 것입니다. Netlib 은 C 와 Fortran 으로 작성된 과학 기술 계산 알고리듬을 방대한 규모로
모아 둔 것으로, 믿기 힘들 만큼의 가치를 가지고 있으며 (오랜 기간의 검증을
통해) 뛰어난 강건성을 보유하고 있습니다. 이런 알고리즘을 다시 작성하는 것은 비현실적이고,
디버깅에만도 수년이 걸릴 수 있습니다. SciPy 는 이러한 알고리듬을 파이썬에서 사용할 수 있도록
그 주위를 감싸는 "래퍼(wrapper)"를 생성하기 위해
다양한 수단을 동원합니다. 일부 래퍼는 C로 직접 손으로 작성하여 만들었으며, 나머지는 SWIG 또는
...
두 번째 답은, 어려운 문제에 대해서는 더 적절한 알고리듬을 적용함으로써
문제를 해결하는 데 걸리는 시간을 획기적으로 개선할 수 있다는 점입니다.
따라서, SciPy의 내장 알고리즘을 사용하시는 편이 단순한 알고리즘을 C로 직접 구현한 것보다
훨씬 빠를 수 있습니다.

## 버그를 발견했습니다. 어떻게 해야 합니까?

SciPy 개발팀은 SciPy 의 신뢰성을 향상시키기 위해 열심히 일하고 있습니다만,
어떤 소프트웨어 제품에서나 그러하듯 버그는 발생합니다. 여러분의 소프트웨어에 영향을 주는 버그를 발견하셨다면, 저희에게 알려주시기
바랍니다. [SciPy 버그 트래커](https://github.com/scipy/scipy/issues) 에 티켓을 입력해 주십시오.

## SciPy에 어떻게 참여할 수 있습니까?

[커뮤니티](/community) 페이지를 방문해 주십시오.
...
이상적인 세계라면, NumPy 는 배열 자료형과 가장 기본적인 연산
(인덱싱, 정렬, 형상 변경, 기초적인 원소별 연산 기능 등)만 담고 있을 것입니다. 모든 수치 계산 코드는 SciPy에 있어야 합니다.
하지만 NumPy 의 중요한 목표 중 하나가 호환성이기 때문에, NumPy 는
NumPy 가 계승하고자 하는 이전 라이브러리에서 지원하던 모든 기능들을
유지하고자 합니다.
...
`scipy.linalg`은 Fortran [LAPACK](https://www.netlib.org/lapack/) 을 보다
완전한 형태로 파이썬에서 사용할 수 있도록 준비한 것으로,
[f2py](https://numpy.org/doc/stable/f2py/index.html)를 사용하여 만든 것입니다.

NumPy 의 설계 목표 중 하나는 Fortran 컴파일러 없이도 이진 파일을 빌드할 수
있게 하는 것이었습니다. LAPACK 을 사용할 수 없다면 NumPy 는 자체적으로
구현된 기능을 사용할 것입니다. SciPy 는 이진 파일 빌드를 위해 Fortran 컴파일러가 필요하며,
내부적으로 Fortran 코드에 크게 의존합니다.
